...
public class SimpleScreens {
     ...
     * @param panel holds the graphics for the off-screen image
     */
	void redrawFinish(MazePanel panel, float energyLevel, int pathLength) {
		Graphics g = panel.getBufferGraphics() ;
        if (null == g) {
        	LOGGER.warning(errorMsg) ; 
        }
        else {
            redrawFinish(g, energyLevel, pathLength);
        }
	}
	...
	/**
	 * New UI code for project 3, redraws finish for if the wizard failed
     * Draws the finish screen, screen content is hard coded
     * @param panel holds the graphics for the off-screen image
     */
	void redrawFinishWithFail(MazePanel panel, float energyLevel, int pathLength) {
		Graphics g = panel.getBufferGraphics() ;
        if (null == g) {
        	LOGGER.warning(errorMsg) ; 
        }
        else {
            redrawFinishWithFail(g, energyLevel, pathLength);
        }
	}
	 ...
	 */
	private void redrawFinish(Graphics gc, float energyLevel, int pathLength) {
		...
		updateFontAndColor(gc, smallBannerFont, MazeColors.TITLE_SMALL);
		centerString(gc, "Congratulations!", 150);
		centerString(gc, "Energy Used : " + energyLevel, 200);
		centerString(gc, "Path Length : " + pathLength, 250);
	...
	/**
	 * New UI code for project 3, redraws finish for if the wizard failed so that it can give fail message
	 * Helper method for redraw with fail to draw final screen, screen is hard coded
	 * @param gc graphics is the off-screen image
	 */
	private void redrawFinishWithFail(Graphics gc, float energyLevel, int pathLength) {
		// produce blue background
		drawBackground(gc);
		// write the title 
		updateFontAndColor(gc, largeBannerFont, MazeColors.TITLE_LARGE);
		...
		centerString(gc, "Energy Used : " + energyLevel, 150);
		centerString(gc, "Path Length : " + pathLength + "\n", 200);
		centerString(gc, "Robot malfunctioned or failed", 250);
		// write the instructions
		gc.setColor(ColorTheme.getColor(MazeColors.TITLE_DEFAULT));
		centerString(gc, "Hit any key to restart", 300);
	}
