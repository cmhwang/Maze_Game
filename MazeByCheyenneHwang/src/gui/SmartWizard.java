...
/**
 * This is a robot driver algorithm, the extra one used for project 4
 * 
 ...
 * difference from normal wizard is that it can use the jump feature and decides when its worth it to jump
 * 
 * Strategy: makes decision whether to jump based on 3 main things: if it is being forced to turn away from the exit because of presence of a wall and it if has enough energy
...
public class SmartWizard extends Wizard{
	...
	public boolean drive1Step2Exit() throws Exception {
		...
		if (robot.hasStopped()) {
			...
			int[] exitPos = maze.getExitPosition();
			...
			int[] exitRelDir = {exitPos[0] - curPos[0], exitPos[1] - curPos[1]};
			...
			if (curCarDir == CardinalDirection.North) {
				...
				if (relDirToTurn[0] == 1) {
					if (exitRelDir[0] == 0 && energyUsed < (float) 3450 && curPos[1] < maze.getHeight()) { 
					// robot on the correct column but is being forced to turn from it
					//if the turn is being forced by a wall then jump otherwise just move 
					// won't invoke if not enough energy
						if (maze.hasWall(curPos[0], curPos[1], curCarDir)) {
							energyUsed = energyUsed + 1;
							robot.jump();
							energyUsed = energyUsed + 40;
					...
					if (exitRelDir[0] == 0 && energyUsed < (float) 3450 && exitRelDir[1] < 0 && curPos[1] != maze.getHeight()) { 
					// robot on the correct column and going the right way but is being forced to turn from it
					//if the turn is being forced by a wall then jump otherwise just move 
					// won't invoke if not enough energy
						if (maze.hasWall(curPos[0], curPos[1], curCarDir)) {
							energyUsed = energyUsed + 1;
				...
				if (relDirToTurn[1] == -1) {
					if (exitRelDir[1] == 0 && energyUsed < (float) 3450 && curPos[0] > 0) { 
					// robot on the correct row but is being forced to turn from it
					//if the turn is being forced by a wall then jump otherwise just move 
					// won't invoke if not enough energy
						if (maze.hasWall(curPos[0], curPos[1], curCarDir)) {
							energyUsed = energyUsed + 1;
					...
					if (exitRelDir[1] == 0 && energyUsed < (float) 3450 && exitRelDir[0] < 0 && curPos[0] > 0) { 
					// robot on the correct row and going the right way but is being forced to turn from it
					//if the turn is being forced by a wall then jump otherwise just move 
					// won't invoke if not enough energy
						if (maze.hasWall(curPos[0], curPos[1], curCarDir)) {
							energyUsed = energyUsed + 1;
				...
				if (relDirToTurn[0] == -1) {
					if (exitRelDir[0] == 0 && energyUsed < (float) 3450 && curPos[1] > 0) { 
					// robot on the correct column but is being forced to turn from it
					//if the turn is being forced by a wall then jump otherwise just move 
					// won't invoke if not enough energy
					if (maze.hasWall(curPos[0], curPos[1], curCarDir)) {
					...
					if (exitRelDir[0] == 0 && energyUsed < (float) 3450 && exitRelDir[1] > 0 && curPos[1] > 0) { 
					// robot on the correct column and going the right way but is being forced to turn from it
					//if the turn is being forced by a wall then jump otherwise just move 
					// won't invoke if not enough energy
						if (maze.hasWall(curPos[0], curPos[1], curCarDir)) {
				...
				if (relDirToTurn[1] == 1) {
					if (exitRelDir[1] == 0 && energyUsed < (float) 3450 && curPos[0] < maze.getWidth() - 1) { 
					// robot on the correct row but is being forced to turn from it
					//if the turn is being forced by a wall then jump otherwise just move 
					// won't invoke if not enough energy
						if (maze.hasWall(curPos[0], curPos[1], curCarDir)) {
					...
					if (exitRelDir[1] == 0 && energyUsed < (float) 3450 && exitRelDir[0] > 0 && curPos[0] < maze.getWidth() - 1) { 
					// robot on the correct row and going the right way but is being forced to turn from it
					//if the turn is being forced by a wall then jump otherwise just move 
					// won't invoke if not enough energy
						if (maze.hasWall(curPos[0], curPos[1], curCarDir)) {
