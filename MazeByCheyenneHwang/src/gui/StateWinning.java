...
public class StateWinning implements State {
    ...
    public void start(Control control, MazePanel panel) {
        ...
        if (control.getDriver() == null) {
        	...
        	view.redrawFinish(panel, control.getDriver().getEnergyConsumption(), control.getDriver().getPathLength());
        }
    ...
    /**
     * Start the game by showing the final screen with a winning message.
     * Same as above version, just with boolean parameter for Wizard as robot driver implementation
     * @param controller needed to be able to switch states, must be not null
     * @param panel is the UI entity to produce the screen on 
     * @param failed checks to see if the Wizard failed, if it did the redraw process needs to change, if true will trigger
     */
    public void start(Control control, MazePanel panel, Boolean failed) {
    	if (failed) {
    		started = true;
            // keep the reference to the controller to be able to call method to switch the state
            this.control = control;
            ...
            // otherwise show finish screen with winning message
            // draw content on panel
            view.redrawFinishWithFail(panel, control.getDriver().getEnergyConsumption(), control.getDriver().getPathLength());
            // update screen with panel content
            panel.update();
    	} else {
    		started = true;
    ...
    public void switchToTitle() {
        ...
        view.redrawGenerating(panel, 0);
        panel.update();
        //ALERT THIS MAY NOT BE NEEDED
